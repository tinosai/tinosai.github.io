...
\begin{equation}
Z \sim \mathcal{N}(\theta,\Sigma_\theta)
\end{equation}
...
\begin{equation}
\theta | Z \sim \mathcal{N}(\mu, \Sigma_\mu)
\end{equation}
...
Before showing the objective function, recall that, for the log posterior:
\begin{equation}
\textrm{logp} \propto \log(\textrm{likelihood}) +  \log(\textrm{prior})
\end{equation}

The objective function to minimize is included below. Note how the unnormalized log-posterior is the sum of three terms:
one for $\sigma^2$ (the prior on the variance parameter is an inverse-Gamma), one for $\beta$ (the prior on the coefficients of the covariates is a multivariate normal) and, finally, the log-likelihood of the model (which is again multivariate normal).
